...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        database = openOrCreateDatabase("ariang.sqlite", MODE_PRIVATE, null);
    ...
    private void addYeuthich() {
        myarray3.clear();
        Cursor c = database.rawQuery("SELECT * FROM AriangSongList WHERE YEUTHICH = 1", null);
        c.moveToFirst();
    ...
    private void addDanhsach() {
        myarray2.clear();
        Cursor c = database.rawQuery("SELECT * FROM AriangSongList", null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
        ...
        edttim.addTextChangedListener(new TextWatcher() {
            ...
            private void getdata() {
                ...
                if (!edttim.getText().toString().equals("")) {
                    Cursor c = database.rawQuery("SELECT * FROM AriangSongList WHERE TENBH LIKE '%" + dulieunhap + "%' OR MABH LIKE '%" + dulieunhap + "%'", null);
                    c.moveToFirst();
                    while (c.isAfterLast() == false) {
    ...
    // Hàm xử lý Copy cơ sở dữ liệu từ thư mục assets vào hệ thống thư mục cài đặt
